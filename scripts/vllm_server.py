...
def create_vllm_engine(model_name, tensor_parallel_size, max_model_len, gpu_memory_utilization):
...
def start_vllm_server(model_name, tensor_parallel_size, max_model_len, gpu_memory_utilization, host, port):
    ...
    print(f"Example: vllm serve {model_name} --tensor-parallel-size {tensor_parallel_size} --host {host} --port {port}")
...
def run_vllm_cli(model_name, tensor_parallel_size, max_model_len, gpu_memory_utilization, host, port):
...
def main():
    parser = argparse.ArgumentParser(description="Unified vLLM Server for Resume Parser")
    parser.add_argument(
        "--model",
        type=str,
    ...
    if args.check_gpu:
        ...
        if gpu_count < args.tensor_parallel_size:
            print(f"Warning: Requested {args.tensor_parallel_size} GPUs but only {gpu_count} available")
        sys.exit(0)
    ...
    gpu_count = check_gpu_availability()
    if gpu_count < args.tensor_parallel_size:
        print(f"Error: Requested {args.tensor_parallel_size} GPUs but only {gpu_count} available")
        sys.exit(1)
