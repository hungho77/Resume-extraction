...
def test_llm_client():
    ...
    # Test vLLM client
    try:
        vllm_client = LLMClient(base_url="http://localhost:8000")
...
def test_resume_processor():
    """Test ResumeLLMProcessor with different configurations"""
    print("\n📄 Testing ResumeLLMProcessor")
    print("=" * 50)
    
    # Test with vLLM
    try:
        vllm_processor = ResumeLLMProcessor(base_url="http://localhost:8000")
        health = vllm_processor.llm_client.health_check()
        print(f"🔍 vLLM processor: {'✅ Available' if health else '❌ Not available'}")
        
        if health:
            # Test specific info extraction
            text = "John Doe is a software engineer with 5 years of experience in Python and JavaScript."
            skills = vllm_processor.extract_specific_info(text, 'skills')
            print(f"💻 Skills extraction: {skills}")
            
            # Test summary generation
            summary = vllm_processor.summarize_resume(text)
            print(f"📝 Summary: {summary[:100]}...")
        
        return health
    except Exception as e:
        print(f"❌ vLLM processor error: {e}")
        return False
...
def test_environment_configuration():
    ...
    try:
        ...
        print(f"🔍 Environment client: {'✅ Available' if health else '❌ Not available'}")
        return health
    except Exception as e:
...
def main():
    ...
    tests = [
        ...
        ('resume_processor', test_resume_processor),
